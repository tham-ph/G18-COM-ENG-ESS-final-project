...
async function showTaskGrid(projectId) {
  ...
  tasks.map(async (task) => {
    ...
    if (taskDoc.exists()) {
      ...
      addTaskToHTML(t.name, t.description, t.status, taskDoc.id, projectId);
    }
  });
...
function manageAddTask(projectId) {
// open add task popup
...
//confirm adding new task
  ...
  confirmAddTaskButton.addEventListener("click", async () => {
    ...
    addTaskToHTML(name, description, status, taskId, projectId);
    addTaskToProjectInFireBase(taskId, projectId);
  });
...
function manageEditTask(task, name, description, status, taskId, projectId) {
  ...
  confirmEditTaskButton.addEventListener("click", () => {
    ...
    editTaskInFirebase(editedName, editedDescription, editedStatus, currentId, projectId);
  });
  ...
  deleteTaskButton.addEventListener("click", () => {
    deleteTaskInFirebase(taskId, projectId);
    deleteTaskInHTML(taskId);
    deleteTaskFromProjectInFirebase(taskId, projectId);
  ...
  statusSelectionButton.addEventListener("change", () => {
    ...
    editTaskInFirebase(name, description, editedStatus, taskId);
    updateProjectPercent(projectId);
  });
...
function addTaskToHTML(name, description, status, taskId, projectId) {
...
async function addTaskToProjectInFireBase(taskId, projectId) {
  const project = await getDoc(doc(db, "projects", projectId));
  const taskList = project.data().taskList;
...
async function deleteTaskFromProjectInFirebase(taskId, projectId) {
  const project = await getDoc(doc(db, "projects", projectId));
  const taskList = project.data().taskList;
